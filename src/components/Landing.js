  ...
  methods: {
    ...
    moverImagen(event) {
      if (this.$refs.imgHover) {
        ...
        this.$refs.imgHover.style.top = `${event.pageY + 10}px`
      }
    },
    mostrarImagen(event) {
      if (this.$refs.imgHover) {
        ...
        setTimeout(() => {
          this.$refs.imgHover.style.opacity = '1' // Luego aplicar la opacidad con transición
        }, 10)
      }
    },
    ocultarImagen() {
      if (this.$refs.imgHover) {
        this.$refs.imgHover.style.opacity = '0'
  ...
  mounted() {
    ...
    window.addEventListener('scroll', function () {
      ...
      if (scrollY < fadeStart) {
        ...
        portfolioImg.style.opacity = '0.08'
      } else {
